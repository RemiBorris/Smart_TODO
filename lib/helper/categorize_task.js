...
async function autoCategorize(taskName) {
  ...
  */
  ...
  await Promise.all(categoryApis)
  .then((values) => {
    if (values[0] && (lowerCaseTaskName.startsWith('watch') || lowerCaseTaskName.startsWith('to watch'))) {
      return result.category_id = 2;
    } else if (values[1] && (lowerCaseTaskName.startsWith('read') || lowerCaseTaskName.startsWith('to read'))) {
      return result.category_id = 1;
    } else if (values[2] && (lowerCaseTaskName.startsWith('eat') || lowerCaseTaskName.startsWith('to eat'))) {
      return result.category_id = 4;
    }
    // else if (values[3] && lowerCaseTaskName.startsWith('buy' || 'purchase') || lowerCaseTaskName.startsWith('to buy' || 'to purchase')) {
    //   result.category_id = 3;
    // }
  })
    */
